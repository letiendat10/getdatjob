...
function normalizeLocation(loc: string): string {
  ...
  if (l.includes("san francisco") || l.includes("bay area") || l.match(/,\s*(ca|california)\s*$/) || l === "ca" || l === "california" || l.includes("palo alto") || l.includes("menlo park") || l.includes("mountain view") || l.includes("sunnyvale") || l.includes("san jose") || l.includes("south san francisco")) return "San Francisco Bay Area";
  if (l.includes("new york") || l === "ny" || l === "nyc" || l.includes("brooklyn") || l.includes("manhattan")) return "New York City";
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          {job.salary_range && <span className="text-xs text-zinc-600 bg-zinc-100 px-1.5 py-0.5 rounded">{job.salary_range}</span>}
          {isVerified && (
            <span className="inline-flex rounded-full p-[2px]" style={{ background: "linear-gradient(90deg,#ff6b6b,#ffd93d,#6bcb77,#4d96ff,#a855f7)" }}>
              <span className="inline-flex items-center rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-zinc-900">Verified LCA Filings With Same Job Title</span>
            </span>
          )}
          {isFriendly && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-medium border border-emerald-200">H-1B Friendly Employer</span>}
          {tnCategory && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 text-xs font-medium border border-blue-200">TN Friendly</span>}
